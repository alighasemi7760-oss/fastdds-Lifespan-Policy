...
if not log_dir:
    print("[-] Error: 'adaptive_metrics.csv' not found in standard paths!")
    print("[!] Please run your DDS simulation binary first to generate CSV logs.")
    sys.exit(1)
...
# ذخیره تصویر در چند مسیر اصلی جهت دسترسی آسان سرور
out_name = 'qos_comparison_ddil_4panels.png'
plt.savefig(os.path.join(log_dir, out_name), dpi=300)
plt.savefig(f'/root/dds_qos_project/{out_name}', dpi=300)

print(f"[+] Plot generated successfully and saved as {out_name}")
