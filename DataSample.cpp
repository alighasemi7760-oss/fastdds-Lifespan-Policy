...
int main(int argc, char** argv) {
    ...
    if (role == "subscriber") {
        ...
        Topic* topic = participant->create_topic("DDIL_Topic", "DataSampleStruct", TOPIC_QOS_DEFAULT);
        ...
        Publisher* publisher = publisher = participant->create_publisher(PUBLISHER_QOS_DEFAULT);
        Topic* topic = participant->create_topic("DDIL_Topic", "DataSampleStruct", TOPIC_QOS_DEFAULT);
        ...
        if (is_adaptive) {
            ...
            writer_qos.lifespan().duration = eprosima::fastrtps::Duration_t(1, 500000000);
